...
import time
import threading
from datetime import datetime, timedelta, timezone # 引入时区处理模块

# --- 页面配置 ---
st.set_page_config(
    page_title="游资捕手 v3.4：北京时间版",
    page_icon="🦅",
    layout="wide",
...
if not raw_df.empty:
    ...
    if last_error:
        status_placeholder.warning(f"⚠️ 数据已展示 (北京时间 {time_str})，但后台最新一次更新遇到问题: {last_error}")
    else:
        # 明确标注为 北京时间
        status_placeholder.success(f"✅ 数据状态健康 | 后台更新时间: {time_str} (北京时间)")
