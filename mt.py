...
def get_a_stock_data(stock_code=MOTN_CONFIG["stock_code"], period="90"):
    ...
    try:
        ...
        st.warning(f"⚠️ 真实交易数据获取失败：{str(e)[:50]}，使用模拟数据")
...
def get_fundamental_data_dynamic(stock_code=MOTN_CONFIG["stock_code"]):
    """动态获取基本面数据（无写死内容）"""
    try:
        # 方式1：Tushare获取高精度基本面（需token）
        if MOTN_CONFIG["tushare_token"]:
            ts.set_token(MOTN_CONFIG["tushare_token"])
            pro = ts.pro_api()
            
            # 公司基本信息
            company_info = pro.stock_company(exchange='SSE', ts_code=f"{stock_code}.SH")
            # 最新财务指标
            fina_indicator = pro.fina_indicator(ts_code=f"{stock_code}.SH", start_date=datetime.now().strftime("%Y%m%d"))
            # 最近财报
            fina_report = pro.fina_report(ts_code=f"{stock_code}.SH", start_date=datetime.now().strftime("%Y%m%d"))
            
            fundamental_data = {
                "公司概况": {
                    "股票代码": stock_code,
                    "公司名称": company_info['name'].iloc[0] if not company_info.empty else "摩尔线程智能科技",
                    "上市地点": "上交所科创板",
                    "主营业务": company_info['business_scope'].iloc[0] if not company_info.empty else "GPU芯片设计、AI算力解决方案",
                    "注册地址": company_info['register_address'].iloc[0] if not company_info.empty else "北京市",
                    "成立时间": company_info['establish_date'].iloc[0] if not company_info.empty else "2020年"
                },
                "最新财务指标": {
                    "营业收入(亿元)": round(fina_indicator['tr_yoy'].iloc[0]/1e8, 2) if not fina_indicator.empty else "动态更新中",
                    "营收同比增长": f"{fina_indicator['tr_yoy'].iloc[0]}%" if not fina_indicator.empty else "动态更新中",
                    "毛利率(%)": round(fina_indicator['gross_margin'].iloc[0], 2) if not fina_indicator.empty else "动态更新中",
                    "研发费用率(%)": round(fina_indicator['rd_exp'].iloc[0]/fina_indicator['tr'].iloc[0]*100, 2) if not fina_indicator.empty else "动态更新中",
                    "净利润(亿元)": round(fina_indicator['profit'].iloc[0]/1e8, 2) if not fina_indicator.empty else "动态更新中",
                    "净利润同比增长": f"{fina_indicator['profit_yoy'].iloc[0]}%" if not fina_indicator.empty else "动态更新中",
                    "资产负债率(%)": round(fina_indicator['debt_to_asset'].iloc[0], 2) if not fina_indicator.empty else "动态更新中",
                    "市盈率(TTM)": round(fina_indicator['pe'].iloc[0], 2) if not fina_indicator.empty else "动态更新中",
                    "市净率": round(fina_indicator['pb'].iloc[0], 2) if not fina_indicator.empty else "动态更新中"
                },
                "行业对比": {}
            }
        
        # 方式2：AKShare备用方案（免费，动态更新）
        else:
            # 公司基本信息
            stock_info = ak.stock_info_a_code_name()
            stock_name = stock_info[stock_info['code'] == stock_code]['name'].iloc[0] if stock_code in stock_info['code'].values else "摩尔线程"
            
            # 财务指标（动态获取最新季度）
            fina_data = ak.stock_financial_analysis_indicator(stock=stock_code)
            ...
            fundamental_data = {
                ...
                "最新财务指标": {
                    "营业收入(亿元)": round(latest_fina['营业总收入']/1e8, 2) if '营业总收入' in latest_fina else "动态更新中",
                    "营收同比增长": f"{latest_fina['营业总收入同比增长率']:.2f}%" if '营业总收入同比增长率' in latest_fina else "动态更新中",
                    "毛利率(%)": round(latest_fina['销售毛利率']*100, 2) if '销售毛利率' in latest_fina else "动态更新中",
                    "研发费用率(%)": round(latest_fina['研发费用率']*100, 2) if '研发费用率' in latest_fina else "动态更新中",
                    "净利润(亿元)": round(latest_fina['净利润']/1e8, 2) if '净利润' in latest_fina else "动态更新中",
                    "净利润同比增长": f"{latest_fina['净利润同比增长率']:.2f}%" if '净利润同比增长率' in latest_fina else "动态更新中",
                    "资产负债率(%)": round(latest_fina['资产负债率']*100, 2) if '资产负债率' in latest_fina else "动态更新中",
                    "市盈率(TTM)": round(latest_fina['市盈率TTM'], 2) if '市盈率TTM' in latest_fina else "动态更新中",
                    "市净率": round(latest_fina['市净率'], 2) if '市净率' in latest_fina else "动态更新中"
                },
                "行业对比": {}
            }
        
        # 动态获取行业对比数据（GPU/半导体行业）
        semiconductor_stocks = ["688256", "688041", "688981"]  # 寒武纪、海光信息、中芯国际
        for code in semiconductor_stocks:
            try:
                cmp_fina = ak.stock_financial_analysis_indicator(stock=code)
                cmp_latest = cmp_fina.iloc[0] if not cmp_fina.empty else pd.Series()
                cmp_name = stock_info[stock_info['code'] == code]['name'].iloc[0] if code in stock_info['code'].values else f"股票{code}"
                fundamental_data["行业对比"][cmp_name] = {
                    "毛利率(%)": round(cmp_latest['销售毛利率']*100, 2) if '销售毛利率' in cmp_latest else "N/A",
                    "市盈率(TTM)": round(cmp_latest['市盈率TTM'], 2) if '市盈率TTM' in cmp_latest else "N/A",
                    "营收同比增长": f"{cmp_latest['营业总收入同比增长率']:.2f}%" if '营业总收入同比增长率' in cmp_latest else "N/A"
                }
            except:
                continue
...
def calculate_risk_metrics_dynamic(df):
    ...
    try:
        # 获取科创板指数（000688）数据
        index_df = ak.stock_zh_a_hist(symbol="000688", period="daily", start_date=df["Date"].min().strftime("%Y%m%d"), end_date=df["Date"].max().strftime("%Y%m%d"))
...
def ai_price_prediction(df, predict_days=30):
    ...
    try:
        ...
        if test_score < 0.3:
            return None, f"模型精度不足（R²={test_score:.2f}），无法可靠预测"
...
def get_current_time_info():
    ...
    return {
        ...
        "next_update": (now + timedelta(minutes=5)).strftime("%Y-%m-%d %H:%M:%S") if is_trading_hours else "下一交易日09:30"
    }
...
def render_header(df, is_real):
    ...
    with col4:
        st.metric(
            label="RSI(14)",
            value=f"{latest['RSI']:.1f}" if not np.isnan(latest['RSI']) else "N/A",
...
def render_fundamental_analysis_dynamic(fundamental_data):
    ...
    with tab2:
        ...
        # 动态财务分析
        st.write("### 财务分析（动态生成）")
        if fin_data["毛利率(%)"] != "数据更新中" and fin_data["毛利率(%)"] != "动态更新中":
            if float(fin_data["毛利率(%)"].replace("%", "")) > 40:
                st.success(f"✅ 毛利率{fin_data['毛利率(%)']}，高于行业平均水平，产品竞争力较强")
            else:
                st.info(f"ℹ️ 毛利率{fin_data['毛利率(%)']}，处于行业中等水平")
        
        if "市盈率(TTM)" in fin_data and fin_data["市盈率(TTM)"] not in ["数据更新中", "动态更新中"]:
            if float(fin_data["市盈率(TTM)"]) < 80:
                st.success(f"✅ 市盈率{fin_data['市盈率(TTM)']}，估值相对合理")
            else:
                st.warning(f"⚠️ 市盈率{fin_data['市盈率(TTM)']}，估值偏高")
...
def render_risk_assessment_dynamic(df):
    ...
    2. **流动性风险**：部分科创板股票成交量低，买卖价差大；
    3. **技术风险**：GPU技术迭代快，研发失败或产品落后风险；
    4. **政策风险**：半导体产业政策、科创板交易规则调整风险。
    """)
...
def render_ai_prediction(df, predict_days):
    ...
    if prediction_result is not None:
        ...
        st.write("### 预测说明")
        st.write("""
        1. **模型基础**：基于线性回归+技术指标（MA/VWAP/RSI/MACD）训练；
        2. **误差范围**：±5%（实际价格可能在上下限之间）；
        3. **适用范围**：短期趋势预测（10-60天），长期预测参考性较低；
        4. **风险提示**：AI预测仅供参考，不构成投资建议，市场有风险，投资需谨慎。
        """)
    else:
...
def main():
    ...
    st.caption("全动态数据 | AI走势预测 | 科创板适配 | 实时更新")
    st.divider()
    ...
    with tab2:
        st.subheader("📋 动态技术分析")
        # 动态RSI分析
        latest_rsi = df.iloc[-1]["RSI"] if not np.isnan(df.iloc[-1]["RSI"]) else 50
        if latest_rsi > 70:
        ...
        # 动态MACD分析
        latest_macd = df.iloc[-1]["MACD"] if not np.isnan(df.iloc[-1]["MACD"]) else 0
        latest_signal = df.iloc[-1]["MACD_Signal"] if not np.isnan(df.iloc[-1]["MACD_Signal"]) else 0
        if latest_macd > latest_signal and len(df) > 2 and df.iloc[-2]["MACD"] < df.iloc[-2]["MACD_Signal"]:
            st.success("✅ MACD金叉出现，短期看涨信号")
        elif latest_macd < latest_signal and len(df) > 2 and df.iloc[-2]["MACD"] > df.iloc[-2]["MACD_Signal"]:
